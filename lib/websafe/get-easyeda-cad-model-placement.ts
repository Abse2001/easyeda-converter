  ...
  const placementPromise = (async () => {
    try {
      ...
      return {
        modelObjUrl,
        bounds,
        // JLCPCB model Z uses the model's absolute minimum Z minus the
        // absolute EasyEDA SVGNODE Z offset (stored in 10-mil units).
        positionZMm: Math.abs(bounds.min.z) - svgNodeZOffsetMm,
      }
    } catch (error) {
      console.error(
